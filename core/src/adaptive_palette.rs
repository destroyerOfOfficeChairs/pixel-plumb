...
pub fn adaptive_palette(
    ...
    // Selection follows the same space as mapping: perceptual (OkLab octree) by
    // default, naive (RGB octree) under --rgb. This keeps the two comparison
    // modes internally consistent — "perceptual end to end" vs "naive end to
    // end" — rather than mixing an OkLab-selected palette with RGB mapping.
